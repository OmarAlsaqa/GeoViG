...
def get_args_parser():
    parser = argparse.ArgumentParser(
        'GeoViG training and evaluation script', add_help=False)
    parser.add_argument('--batch-size', default=128, type=int)
    parser.add_argument('--epochs', default=300, type=int)

    # Model parameters
    parser.add_argument('--model', default='geovig_s', type=str, metavar='MODEL',
                        help='Name of model to train (e.g. geovig_s, geovig_ti, ...)')
    ...
    parser.add_argument('--data-path', default='./imagenet', type=str,
                        help='dataset path')
    parser.add_argument('--data-set', default='IMNET', choices=['CIFAR', 'CIFAR100', 'IMNET', 'IMNETTE', 'INAT', 'INAT19'],
    ...
    parser.add_argument('--crop_pct', default=0.9, type=float,
                        help='Crop percentatege to use during validation.')
    ...
    parser.add_argument('--attention-type', type=str, default='cross_attention',
                        help='Type of attention for Hierarchical GNN (cross_attention or gat)')
    return parser
...
def main(args):
    ...
    if args.distillation_type != 'none':
        ...
        teacher_ckpt = checkpoint['model']
        teacher_model_dict = teacher_model.state_dict()
        for k in list(teacher_ckpt.keys()):
            if k in teacher_model_dict and teacher_ckpt[k].shape != teacher_model_dict[k].shape:
                print(f"Removing key {k} from teacher checkpoint due to shape mismatch")
                del teacher_ckpt[k]
        ...
        if not args.eval and 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint:
            ...
            if args.model_ema:
                utils._load_checkpoint_for_ema(
                    model_ema, checkpoint['state_dict_ema'])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        'GeoViG training and evaluation script', parents=[get_args_parser()])
